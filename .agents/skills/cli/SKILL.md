...
description: >-
  ...
  report charts. Use when the user asks to prepare CSVs, run agent-turn or
  session-span, chart a report, or how to invoke `bun cli`.
---
...
bun cli experiments session-span charts --no-open
```
...
| Chart reports | [docs/how-to/chart-reports.md](../../../docs/how-to/chart-reports.md) |
...
4. Charts go through `experiments <name> charts` (per-experiment `renderCharts`), not a shared report viewer.
5. When adding CLI flags or changing prepare/experiment behavior, update the matching **how-to + reference** pages (and READMEs if entrypoints change) using the [documentation](../agent-review/documentation/SKILL.md) skill.
