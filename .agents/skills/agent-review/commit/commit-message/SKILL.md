...
Then `git commit` yourself. Never ask review/analyst agents to mutate git.

When planning work, PRs, or commit messages in this repository, follow Conventional Commits 1.0.0 below.

Apply going forward; do not rewrite published history. Descriptions are a short imperative summary (prefer lowercase). Spec-mandated types are `feat` and `fix`. Other types MAY be used; this repo uses the Angular set when they fit: `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`, `revert`.

The remainder of this file is the Conventional Commits 1.0.0 specification (https://www.conventionalcommits.org/en/v1.0.0/).
